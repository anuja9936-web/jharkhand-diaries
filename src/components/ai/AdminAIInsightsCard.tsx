...
import { generateGovernmentInsights } from '../../services/ai/aiService';
...
export function AdminAIInsightsCard({
  ...
  const insights = generateGovernmentInsights({
    totalDestinations,
    totalProviders,
    totalOfferings,
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h3 className="font-display text-base font-bold text-ink-950 flex items-center gap-2">
              <span>Tourism Intelligence &amp; AI Insights</span>
              <Badge variant="accent" className="text-[10px] py-0.5">
                AI Governance
              </Badge>
            </h3>
            <p className="text-xs text-ink-500">
              Real-time analytics synthesis across 24 districts, provider growth, and visitor safety alerts
            </p>
          </div>
        </div>
        ...
        {insights.map((item, idx) => (
          ...
          >
            ...
            <div className="pt-2 border-t border-black/5 text-[11px] font-medium text-forest-900 bg-forest-100/50 -mx-4 -mb-4 p-3 rounded-b-2xl flex items-start gap-1.5">
              ...
              <span><strong>Recommendation:</strong> {item.actionRecommendation}</span>
            </div>
          </div>
        ))}
      </div>
